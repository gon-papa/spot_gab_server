...
class PasswordResetVerifications(SQLModel, table=True):
    ...
    user_id: int = Field(
        default=None,
        sa_column=Column(Integer, ForeignKey("users.id"), nullable=False, unique=True, comment="ユーザーID"),
    )
    ...
    verified_expired_at: Optional[datetime] = Field(
        sa_column=Column(TIMESTAMP(True), nullable=True, comment="パスワードリセット有効期限")
    )
